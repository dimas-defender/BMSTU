...
original - указатель на место, куда будет записан адрес перехватываемой функции
...
/* записать адрес перехватываемой функции */
		...
		struct ftrace_ops *ops, struct ftrace_regs *fregs)
{
	...
	/* получить адрес struct ftrace_hook по адресу struct ftrace_ops */
	struct ftrace_hook *hook = container_of(ops, struct ftrace_hook, ops);

#if USE_FENTRY_OFFSET
	regs->ip = (unsigned long)hook->function;
#else
	/* пропускаем вызовы функции из текущего модуля для избежания беск.цикла */
	if (!within_module(parent_ip, THIS_MODULE))
		regs->ip = (unsigned long)hook->function;
#endif
}

/* регистрация и включение перехвата */
int install_hook(struct ftrace_hook *hook)
{
	...
	hook->ops.flags = FTRACE_OPS_FL_SAVE_REGS
	                | FTRACE_OPS_FL_RECURSION  //disable ftrace's anti-recursion guard
	                | FTRACE_OPS_FL_IPMODIFY;  //для изменения регистра IP
...
/* выключение и отмена регистрации перехвата */
...
#if !USE_FENTRY_OFFSET
#pragma GCC optimize("-fno-optimize-sibling-calls") //disable tail call optimization
#endif
...
#else
// Указатель на оригинальный обработчик системного вызова
