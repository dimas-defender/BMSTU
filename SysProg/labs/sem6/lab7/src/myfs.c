...
MODULE_DESCRIPTION("My file system");

#define MYFS_MAGIC_NUMBER 0xCADD
...
{
	if (cached == MAX_CACHE_SIZE)
		return NULL;

	return myfs_inodes[cached++] = kmem_cache_alloc(myfs_inode_cachep, GFP_KERNEL);
}
...
static struct inode *myfs_make_inode(struct super_block *sb, int mode)
{
	...
	if (ret)
    {
		...
		if ((myfs_inodep = cache_get_inode()) != NULL)
        {
...
static int __init myfs_init(void)
{
	...
	if (!(myfs_inodes = kmalloc(sizeof(struct myfs_inode *) * MAX_CACHE_SIZE, GFP_KERNEL)))
    {
