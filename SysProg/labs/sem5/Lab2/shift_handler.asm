...
data segment para 'data'
    ...
    idt label byte
	idescr_0_12 idescr 13 dup (<0,code2s,0,8Fh,0>)
    idescr_13 idescr <0,code2s,0,8Fh,0>
    idescr_14_31 idescr 18 dup (<0,code2s,0,8Fh,0>)
    int08 idescr <0,code2s,0,10001110b,0> ; p = 1 dpl = 00 0 тип = 1110 (прервания)
    int09 idescr <0,code2s,0,10001110b,0>
    idt_size = $-idt 
    ...
    protectmode  db  'Welcome to Protected Mode!'
    protectmode_len=$-protectmode
    mem_msg db 'Available memory: '
    mem_msg_len=$-mem_msg
    rm_msg      db 27, '[29;44mYou are in Real Mode! Press any key to enter protected mode.', 27, '[0m$'
    pm_msg_out  db 27, '[29;44mBack in Real Mode!', 27, '[0m$'
    color=17h
	color_black=0h
	shift db 0
...
code2 segment para public 'code' use32
    ...
    keybInt proc uses eax ebx edx  ; Обработчик прерывания от клавиатуры
        xor eax, eax
        in  al, 60h
        cmp al, 1Ch
        jne print
...
start:
		...
		lea eax, es:dummy
		load_idt idescr_0_12
    
		lea eax, es:dummy
		load_idt idescr_14_31
    
		lea eax, es:except_13
		load_idt idescr_13

		lea eax, es:timerInt
		load_idt int08 ; прерывание от таймера

		lea eax, es:keybInt
		load_idt int09 ; прерывание от клавиатуры
