...
namespace flexlist {
    ...
    template <typename T>
    Iterator<T> Iterator<T>::operator+(const Iterator<T>& a, difference_type<T> b) {
        return Iterator<T>(a) += b;
    }

    template <typename T>
    Iterator<T> Iterator<T>::operator+(difference_type<T> a, const Iterator<T>& b) {
        return b + a;
    }

    template <typename T>
    void Iterator<T>::swap(Iterator<T>& a, Iterator<T>& b) {
        std::swap(a.node, b.node);
    }

    template <typename T>
    bool Iterator<T>::operator==(const Iterator<T>& a, const Iterator<T>& b) {
